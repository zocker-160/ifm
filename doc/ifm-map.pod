...
the 'boxes-and-lines' method.  Here is an example map to represent using
B<IFM> commands.
...
To draw the example map, you first choose an arbitrary start location: the
kitchen (when mapping a real game, you'd choose your actual start
location).  To add the kitchen, just type this:
...
could never have two distinct rooms with the same name.
...
This is indicated on the map, probably by an arrow.
...
displayed (probably by a small line poking out of the room in those
directions).
...
Some maps can be very large, and may not look good on a single sheet.
...
taken that into account.  To extend the length of a link, just duplicate
the direction indicator in the 'dir' clause (e.g. 'dir e e' instead of 'dir
e').
