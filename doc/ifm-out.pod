...
If you assign a style (say, called B<newstyle>) to a room or link, but
don't define it anywhere in your input, then B<IFM> will look for a file
called F<newstyle.ifm> using the standard search path.  If the file exists,
it is expected to define style B<newstyle>.  For example, you could put the
"water" style definition above into a file called F<water.ifm> somewhere on
the B<IFM> search path, and it would be read automatically.  This is
useful if, for example, you want to use the same style in several different
maps.
