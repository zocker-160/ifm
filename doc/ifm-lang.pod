...
This section gives a formal description of the B<IFM> input language.  In
the sections below, the following symbols are used:
...
=item dir COMPASS [INTEGER] [COMPASS [INTEGER]...] [from ID]
...
using them.  Following a direction with an integer means to repeat it that
many times.
...
Indicate that there is an unsolved puzzle in this room.

=item special

Indicate that the direction link to this room is special.
...
Indicate that this link is special.
...
=item cmd STRING [INTEGER]

Specify the exact command you type to do the task.  If an integer follows
...
the following syntax:
    ...
    [FORMAT] IDENT = INTEGER | REAL | STRING | undef;
...
the variable, and it can take an integer, real or string value.  Note that
setting a variable to the value 'undef' effectively removes it, and allows
the default setting to reappear.

Note that it is not an error to refer to an unknown output format or
variable.  This is so that maps using new formats will continue to work
with old versions of B<IFM>.  One slight drawback of this is that if you
mistype a format or variable there will be no obvious way of telling -- and
no effect on your output either.
