...
Give the map a title.  This command can appear anywhere, and has the same
effect each time.

=item map "<name>"

Indicate that the current room starts a new map section with the given
name.  This is useful if it seems like the map divides into different
sections (e.g. the floors of a house).  But it can lead to errors if you
move from one section to another via a room that hasn't had 'map' declared
on it, since B<ifm> will try to link rooms on different map sections.
...
Declare an item in the current room.  If you don't give it a tag attribute,
one is generated automatically by changing all invalid tag characters in
...
you screw up a command, you'll have to manually edit the resulting
...
Regexp matching commands that aren't allowed (case-insensitive).
