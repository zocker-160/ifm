...
session use B<scr2ifm> to get a map.  Well, that's the theory anyway.  For
best results, you should follow a few simple guidelines when creating your
transcript (see L<"HINTS AND TIPS">).
...
Output from B<scr2ifm> is as follows.  First, there's a list of B<ifm>
commands derived from the transcript.  Each time a new room is seen, a room
...
parameters that control how the transcript is parsed (see below).
...
in the command file instead.  E.g. 'option i' will indent output nicely.

=back

Here's a list of the parameters that control how the transcript is parsed,
and their defaults.  You can use the B<scr2ifm> 'eval' command to set
them in the command file.

=over 4

=item $name_remove (default: C<'\s+\(.+\)'>)
...
=item $name_maxwords (default: C<8>)

Maximum no. of words in a room name.

=item $name_maxuncap (default: C<3>)

Maximum length of an uncapitalized word in a room name.

=item $name_invalid (default: C<'[.!?]'>)

Regexp which, if matched, signals an invalid room name.

=item $name_ignore (default: unset)

Optional regexp matching any other possible room names that should be
ignored.  You can use this for special cases, where particular transcript
lines are thought to be room names when they shouldn't be.

=item $desc_minwords (default: C<20>)

Minimum no. of matching words required to match a room description.

=item $cmd_prompt (default: C<'^E<gt>\s*'>)

Regexp matching a command prompt.

=item $cmd_look (default: C<'^l(ook)?$'>)

Regexp matching a 'look' command (case-insensitive).

=item $cmd_undo (default: C<'^undo$'>)

Regexp matching an UNDO command (case-insensitive).  It's assumed that only
a single level of UNDO is available.

=item $cmd_teleport (default: C<'^(restart|restore)$'>)
