...
you change your initialisation file (see below) the changes won't be
...
=item Debugging output menu

This contains a single menu item, which displays some debugging information
in another window.

=item Help menu

At the moment this just displays a small info panel about the current
B<ifm> version.
...
variable to zero (see L<"CUSTOMIZATION">).
...
variables, their format and defaults.
    ...
    # Debugging output window dimensions.
